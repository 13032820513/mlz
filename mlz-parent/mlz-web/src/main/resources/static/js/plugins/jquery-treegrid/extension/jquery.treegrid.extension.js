			...
			$.each(options.columns, function(index, column) {
				...
				if(index==0&&column.field=='selectItem'){
					...
					if(column.radio){
						...
						_ipt = $('<input name="select_item" type="radio" value="'+item[options.id]+'"></input>');
						td.append(_ipt);
					} 
					if(column.checkbox){
                        var _ipt= "";
                        if(item[options.id] != undefined)
						 _ipt = $('<input name="select_item" type="checkbox" value="'+item[options.id]+'"></input>');
						td.append(_ipt);
					}
					tr.append(td);
				}else{
					var td = $('<td style="'+((column.width)?('width:'+column.width):'')+';text-align:'+ column.align +'"></td>');
                    // 增加formatter渲染
                    if (column.formatter) {
